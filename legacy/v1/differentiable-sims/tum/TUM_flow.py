...
class TUM_flow(World):
    ...
    def __init__(self, params):
        ...
        self.fluid = self.add(Fluid(Domain(resolution=[2*self.res, 3*self.res], box=box[0:200, 0:300], boundaries=CLOSED), buoyancy_factor=buoyancy), physics=IncompressibleFlow())
    ...
    def set_target(self, img_path):
        ...
        val1 = np.max(self.fluid.density.data[0:,...].eval())
    ...
    def train(self):
        ...
        self.session.initialize_variables()
        print('Initial loss: %f' % self.session.run(loss)) #previous states is the lower res
    ...
    def get_optimized_velocities(self):
        ...
        for i in range(self.frames):
            optimized_velocities.append(self.session.run(self.states[i].velocity))#.at_centers())

        print("Got optimized velocities.")
    ...
    def get_densities(self):
        ...
        for i in range(self.frames):
            densities.append(self.session.run(self.states[i].density))
